...
#pragma region constructors
	...
	explicit constexpr text_view(const codeunit_sequence_view& view) noexcept
		: view_(view)
	{ }
	explicit text_view(const codeunit_sequence& sequence) noexcept
		: view_(sequence.view())
	{ }
...
{
	[[nodiscard]] constexpr easy::text_view operator""_txtv(const char* str, const size_t len) noexcept
	{
		return { str, len };
	}
}
