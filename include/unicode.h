...
namespace ostr
{
	namespace unicode
	{
		...
		[[nodiscard]] constexpr std::array<char16_t, utf16::SEQUENCE_MAXIMUM_LENGTH> utf8_to_utf16(char const* const utf8, const u64 length) noexcept
		{
			return utf32_to_utf16(utf8_to_utf32(utf8, length));
		}
		
		[[nodiscard]] constexpr std::array<char, UTF8_SEQUENCE_MAXIMUM_LENGTH> utf16_to_utf8(char16_t const* const utf16, const u64 length) noexcept
		{
	...
	struct codepoint
	{
		...
		explicit constexpr codepoint(const char* const v, const u8 length) noexcept
			: sequence{ }
		{
